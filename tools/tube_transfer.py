...
class Circuit(object):
    ...
    def write_rust_ftube_table(self, Ri, Vi, Vp):
        """write Rust source"""
        sys.stdout.write("TubeTable {\n");
        sys.stdout.write('    min_vi: %g,\n' % (Vi[0],))
        sys.stdout.write('    max_vi: %g,\n' % (Vi[-1],))
        sys.stdout.write('    _mapping: %g,\n' % ((len(Vi)-1)/(Vi[-1]-Vi[0]),))
        sys.stdout.write('    table: [\n')
        for i, v in enumerate(Vi):
            sys.stdout.write('        ' + str(Vp[i]) + ',\n')
        sys.stdout.write("    ]\n};\n")

    def write_rust_tables(self, prgname):
        sys.stdout.write("// generated by %s\n" % prgname)
        sys.stdout.write("// tube: %s\n" % self.tube)
        sys.stdout.write("// plate current function: %s\n" % self.ipk_func)
        for n in self.used_names:
        ...
        for Ri in self.Ri_values:
            sys.stdout.write("\nconst TUBE_%s_RI%s: TubeTable = "
			% (self.tube, self.R_name(Ri).upper()))
            Vp = self.FtubeV(self.Vi, Ri)
            self.write_rust_ftube_table(Ri, self.Vi, Vp)
        sys.stdout.write("\n")
...
def usage():
    print("usage: %s plot|accuracy|table|rust-table|vk0 tube-name plate-func" % sys.argv[0])
    print(Circuit.help())
    raise SystemExit(1)
...
def main():
    ...
    if cmd == "plot":
        ...
        c.write_rust_tables(sys.argv[0])
    elif cmd == "vk0":
